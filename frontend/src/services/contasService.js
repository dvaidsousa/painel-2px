...
export const listarContas = async () => {
  ...
  const response = await fetch('http://localhost:3000/api/contas/listar', {
    headers: {
...
export async function editarLinha(id, dados) {
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/contas/editar/${id}`, {
      method: 'PUT',
...
export async function deletarLinha(id) {
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/contas/deletar/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json', // Define o tipo de conteúdo como JSON
        'Authorization': `Bearer ${token}`, // Adiciona o token no cabeçalho
...
export async function adicionarEntrada(dados) {
  try {
    ...
    const response = await fetch('http://localhost:3000/api/contas/adicionar', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json', // Define o tipo de conteúdo como JSON
        'Authorization': `Bearer ${token}`, // Adiciona o token no cabeçalho
